 ...
 const [input,setInput] = useState("")
  ...
  // handle sending an image fn
   const handleSendImg = async(e)=>{
      ...
      reader.onloadend=async()=>{
        ...
        // make sure to resest the file input as empty 
        e.target.value=''
      }
  ...
  return (      
      selectedUser ? (   
        <div  className='h-full col-span-2 overflow-scroll  relative backdrop-blur-lg'>
           ...
           <div className='flex items-center px-5 py-3 gap-2 border-b border-gray-300'>
            ...
            <div className='text-lg flex items-center gap-2'>{selectedUser.fullName}
         ...
         <div className='flex flex-col h-[calc(100%-20%)] overflow-y-scroll p-3 pb-6'>
     ...
     <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
      ...
      <input type="text" value={input} onChange={(e)=>setInput(e.target.value)} onKeyDown={(e)=>e.key=="Enter"?handleSendMessage():null} placeholder='Send a message' className='flex-1 border-none outline-none text-white p-3 placeholder-gray-400'  />
      <input type="file" onChange={()=>handleSendImg()} id='image' accept='image/png, image/jpeg' hidden/>
       <label htmlFor="image">
        <img src={assets.gallery_icon} alt="gallery-icon" className='w-5 mr-2 cursor-pointer' />
       </label>
      </div>
        <img src={assets.send_button} onClick={()=>handleSendMessage()} alt="send-icon" className='w-9 cursor-pointer' />
     </div>
